...
class TrainingViewModelTest {
    ...
    fun trainingList_returnsTrainingListFromRepository(){
        ...
        val trainingList = listOf(
            TrainingItem(
                id = expectedTrainings[0].id,
                name = expectedTrainings[0].name,
                defaultTimeSec = expectedTrainings[0].defaultTimeSec,
                defaultRestSec = expectedTrainings[0].defaultRestSec,
                lastUsed = Date().time
            ),
            TrainingItem(
                id = expectedTrainings[1].id,
                name = expectedTrainings[1].name,
                defaultTimeSec = expectedTrainings[1].defaultTimeSec,
                defaultRestSec = expectedTrainings[1].defaultRestSec,
                lastUsed = Date().time
            )
        )
    ...
    fun update_callsRepositoryInsert(){
        ...
        val trainingItem = TrainingItem(
            ...
            defaultRestSec = 5,
            lastUsed = 1)
        whenever(clock.timestapm()).thenReturn(1)
